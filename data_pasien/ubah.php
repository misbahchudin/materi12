<?php 
 ...
 $query   = "SELECT * FROM tbl_pasien WHERE no_rm = '$tempKODE'";
 ...
 while ($row = mysqli_fetch_array($sambung))
 {
  ...
  $nm_psn  = $row["nm_psn"];
  $sex     = $row["sex"];
  $tgl_lhr = $row["tgl_lhr"];       // $photo = $row["photo"];    tidak bisa
 }
 ...
 if (isset($_POST["cmdSIMPAN"]))
 {
  ...
  if (strlen(trim($namafile)) <> 0 && !in_array($EXT, $tipefile))
  {
  ?>
   <script>
  ...
  elseif (strlen(trim($namafile)) <> 0 && ($ukuranfile > 1048576 || $ukuranfile == 0))
  {      // directory xampp->php->php.ini  ->  "upload_max_filesize" DEFAULT 2M
   ?>
   <script>
  ...
  else
  {
   ...
   $query   = "UPDATE tbl_pasien SET no_rm = '$no_rm', nm_psn = '$nm_psn', 
              sex = '$sex', tgl_lhr = '$tgl_lhr', photo = '$photo' 
              WHERE no_rm = '$tempKODE'";
   $sambung = mysqli_query($theLINK, $query);
...
?>
<div class="card mt-3">
 <div class="card-header bg-primary text-white">Ubah Data Dosen</div>
  <div class="card-body">
   <form action="" method="post" enctype="multipart/form-data">
   <input type="hidden" name="tempKODE" id="tempKODE" value="<?= $tempKODE; ?>">
   <input type="hidden" name="tempGAMBAR" id="tempGAMBAR" value="<?= $tempGAMBAR; ?>">
   <!-- *********************************************************************************** -->
   ...
   <!-- *********************************************************************************** -->
    <div class="form-group">
     <label for="nm_psn">NAMA LENGKAP PASIEN</label>
   ...
   <!-- *********************************************************************************** -->
    <div class="form-group">
     <label for="photo">
      PHOTO PASIEN <font color="red"><b>(biarkan kosong jika tidak ingin diganti)</b></font>
     </label>
